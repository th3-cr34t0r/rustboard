...
pub(crate) struct BatteryService {
    ...
    #[descriptor(uuid = descriptors::MEASUREMENT_DESCRIPTION, name = "battery_level", read, value = "Battery Level", type = &'static str)]
...
pub(crate) struct HidService {
    ...
    #[characteristic(uuid = "2a4b", read, value = KeyboardReport::desc().try_into().expect("Failed to convert KeyboardReport to [u8; 69]"), permissions(encrypted))]
...
pub(crate) struct SplitService {
    #[characteristic(uuid = SPLIT_REPORT_CH, read, notify, write_without_response, permissions(encrypted))]
    pub(crate) registered_keys: [u8; 6],
    #[descriptor(uuid = descriptors::VALID_RANGE, read, value = [0, 100])]
    #[descriptor(uuid = descriptors::MEASUREMENT_DESCRIPTION, name = "battery_level", read, value = "Battery Level", type = &'static str)]
    #[characteristic(uuid = SPLIT_BATTERY_CH, read, notify, write_without_response, value = 0, permissions(encrypted))]
    pub(crate) level: u8,
}
