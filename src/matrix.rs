...
#[cfg(feature = "col2row")]
impl<'a> Matrix<'a> {
    pub fn init(rows: [Input<'a>; ROWS], cols: [Output<'a>; COLS]) -> Self {
        Self {
            rows,
            cols,
            keys: [MatrixKey::default(); MATRIX_KEYS_BUFFER],
    ...
    async fn enter_async_interrupt(&mut self) {
        if self.keys.iter().all(|key| key.keypos == KeyPos::default()) {
            ...
            // set rows wait for high
            let mut futures: Vec<_, ROWS> = self
                .rows
                .iter_mut()
                .map(|row| row.wait_for_high())
                .collect();
            ...
            {
                Either::First(_) => {
                    // key has been pressed, but first set all cols to low
                    for col in self.cols.iter_mut() {
                        col.set_low();
                    }
                }
    ...
    pub async fn scan(&mut self) {
        ...
        loop {
            // run matrix scan
            for (col_count, col) in self.cols.iter_mut().rev().enumerate() {
                col.set_high();
                // delay so port propagates
                delay_us(10).await;
                ...
                // we aim at 1ms scan interval
                delay_us(1000 / COLS as u64).await;
            }
